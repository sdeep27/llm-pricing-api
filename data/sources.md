...
- `web_search_20260209` dynamic filtering supported on: Claude Fable 5.1, Claude Mythos 5.1, Claude Fable 5, Claude Opus 5, Claude Opus 4.8, Claude Mythos 5, Claude Mythos Preview, Opus 4.7, Opus 4.6, Claude Sonnet 5, Sonnet 4.6 (Anthropic's docs now phrase this as "Claude 4.6 and later models and Claude Mythos Preview")
...
Three of four providers use a 0.1x multiplier for cache reads (90% discount on standard input price) on most models; xAI and the newest Anthropic models deviate. This is stored as `cache_read_price` in pricing.json.

- **Anthropic**: 0.1x base input on all models EXCEPT Claude Fable 5.1 and Claude Mythos 5.1, which are 0.025x ($0.25 on $10 input — confirmed 2026-09-04, see gotcha 32). Also has cache write costs (1.25x for 5-min, 2x for 1-hour) but we only track the read price.
...
- **Anthropic**: Varies significantly (4K for Haiku 3 up to 128K for Opus 4.7/4.6/Opus 5/Fable 5.1). Check the models overview page (`platform.claude.com/docs/en/about-claude/models/overview`) — its compare table lists max output only for the current lineup (Fable 5.1, Opus 5, Sonnet 5, Haiku 4.5 as of 2026-09-04); "legacy" models like Fable 5 aren't in the table, so their max_output stays null unless found elsewhere.
...
21. **xAI Grok 4.20 naming split (flagged 2026-08-13, unresolved)**: `docs.x.ai/developers/models` now lists three separate rows — `grok-4.20-0309-reasoning`, `grok-4.20-0309-non-reasoning`, `grok-4.20-multi-agent-0309` — all with identical pricing to what we store as "Grok 4.20" and "Grok 4.20 Multi-Agent". Unclear whether this is a genuine product split or just an internal model-id/date-suffix convention. Left unchanged pending a clearer page layout; re-check next audit before renaming or splitting rows. (Recurred again 2026-08-25, still identically priced, still unresolved. Recurred again 2026-08-31, still identically priced, still unresolved. Recurred again 2026-09-01, still identically priced, still unresolved. Recurred again 2026-09-04, still identically priced; this run's broad extraction claimed "grok-4.20 variants deprecated per documentation" but a targeted re-fetch quoting the rows found NO deprecation/legacy/retired label on any of the three — the only deprecation notice on the page is for `grok-voice-think-fast-1.0`. Treated as an extraction hallucination; no `deprecated` flag set. Recurred again 2026-09-07, still identically priced, no deprecation label on any of the three — the only deprecation notice on the page is again `grok-voice-think-fast-1.0`. Recurred again 2026-09-10, same.)
...
25. **xAI batch/search/X-search/collections columns absent again (2026-08-16)**: Recurred for the ~6th consecutive audit (previously 07-25, 07-28, 07-31, 08-01, 08-13). Treat as a stable extraction/layout limitation of `docs.x.ai/developers/models`, not a pricing change — carried forward stored values again. (Recurred again 2026-08-19, ~7th consecutive audit; and again 2026-08-22, ~8th; and again 2026-08-25, ~9th consecutive audit; and again 2026-08-31, ~10th consecutive audit; and again 2026-09-01, ~11th consecutive audit — this run's fetch explicitly stated "Batch API, Web Search, X Search, and Collections Search pricing were not detailed on this page."; and again 2026-09-04, ~12th consecutive audit, "Not specified" for all four; and again 2026-09-07, ~13th consecutive audit, "not specified" for all four; and again 2026-09-10, ~14th consecutive audit, "not present" for all four.)

26. **OpenAI GPT-5.3 Codex still absent from pricing table extraction (2026-08-16)**: Recurred again (previously 07-25, 07-28, 07-31, 08-01). Codex continues to sit in a "Specialized models" section separate from the main pricing table that our fetch prompt targets. Carried forward stored values; no evidence of an actual price change. (2026-08-19: the model row itself was captured this run — $1.75/$0.175/$14.00 in/cache/out, exact match to stored, confirming no drift — but the batch columns were still absent, consistent with codex's documented no-discount batch policy. Whether the row appears seems to depend on fetch-prompt phrasing/extraction luck rather than actual page changes. 2026-08-25: row captured again, still an exact match to stored, batch columns still absent — same pattern. 2026-08-31: row captured again, still an exact match to stored ($1.75/$0.175/$14.00), batch columns still absent — same pattern continues. 2026-09-01: row captured again under "Specialized models", still an exact match to stored ($1.75/$0.175/$14.00), batch columns still absent — same pattern continues. 2026-09-04: same — row captured under "Specialized models", exact match, batch absent; the page also now lists a codex "Fast mode" tier at $3.50/$0.35/$28, not tracked. 2026-09-07: same — row captured under "Specialized models", exact match, batch absent, fast mode $3.50/$28 still listed. 2026-09-10: same.)
...
28. **GPT-5.6 Sol price cut, confirmed 2026-08-22 via two independent targeted re-fetches**: Standard (short-context) rate dropped from $5/$0.50/$30 (in/cache/out) to $4/$0.40/$20; batch dropped from $2.50/$15 to $2/$10. Terra and Luna were re-checked in the same run and matched stored values exactly (no change), which is why Sol's mismatch was treated as a real price cut rather than an extraction fluke — see gotcha 18 for the updated LC-tier figures. **2026-09-10 update: the cut is promotional.** OpenAI's page now carries a note below the standard table: "GPT-5.6 Sol's promotional pricing is available at least through November 21, 2026." Same pattern as Gemini 3.6/3.7/3.8 Flash (gotcha 23): we store the current promotional rate ($4/$0.40/$20, batch $2/$10). No reversion price is published, but the pre-cut rate was $5/$0.50/$30 (batch $2.50/$15). Re-check after 2026-11-21 and don't "fix" the price upward before the page actually changes.

29. **Artificial Analysis now surfaces only variant-suffixed entries for some models (2026-08-22)**: A targeted re-check for exact base names ("Claude Sonnet 5", "Claude Fable 5", etc.) found only qualified rows — e.g. "Claude Sonnet 5 (max)" [55], "Claude Sonnet 5 (Non-reasoning)" [43], "Claude Fable 5 (with fallback)" [62] — no unqualified base-name row present. Since it's unclear which variant (if any) corresponds to our stored convention, no scores were rewritten from these reads; treat any single variant-suffixed AA score as manual review, not a source of truth, until a plain base-name row reappears. (Recurred 2026-08-25: only "Claude Opus 5 (max)" [63], "Claude Opus 5 (xhigh)" [63], "Claude Opus 5 (high)" [61], "Claude Fable 5 (with fallback)" [62], "Claude Sonnet 5 (max)" [55] surfaced — still no plain base-name rows; no scores rewritten. Recurred again 2026-08-31: identical pattern — "Claude Opus 5 (max)" [63], "Claude Opus 5 (xhigh)" [63], "Claude Opus 5 (high)" [61], "Claude Fable 5 (with fallback)" [62], "Claude Sonnet 5 (max)" [55] — still no plain base-name rows; no scores rewritten. Recurred again 2026-09-01: same pattern plus "Claude Opus 5 (medium)" [59] and "Claude Opus 5 (low)" [52] also surfaced — still no plain base-name rows; no scores rewritten. Note the variant scores are internally consistent with our stored non-provisional scores: Opus 5 (high)=61 matches our stored Opus 5 score exactly, and Fable 5 (with fallback)=62 vs. our stored 65 — a real but unlabeled discrepancy we're deliberately not chasing until a plain base-name row exists. Recurred again 2026-09-04, now extending beyond Claude: "Claude Fable 5.1 (max with fallback)" [66] / (xhigh) [65] / (high) [62] / (medium) [60] / (low) [58]; "GPT-6 Astra (max)" [61] / (xhigh) [61] / (high) [60] / (medium) [59] / (low) [57] / (Non-reasoning) [55]; "Gemini 3.8 Flash (high)" [59] / (medium) [57] / (low) [52]; "Grok 4.5 (high)" [56] vs stored 54 — none with a plain base-name row, so the three new models got provisional scores instead. Exception this run: a PLAIN "Gemini 3.6 Flash" row surfaced at 52 vs stored 50 (the 08-01 resolution already flagged 50 as a bias-era artifact). Recorded as a FIRST observation only, per the two-consecutive-observation precedent used for every prior score rewrite — apply 50→52 next run if a plain row shows 52 again. 2026-09-07: the 50→52 candidate is SUPERSEDED — the plain "Gemini 3.6 Flash" row now reads 40, because the entire leaderboard shifted down ~9–12 points this run; see gotcha 34. Still no plain base-name rows for Claude/GPT-6/Gemini 3.8/Grok 4.5. Recurred again 2026-09-10: still no plain rows for Fable 5.1 / Fable 5 / Opus 5 / Sonnet 5 / Mythos 5.1 / GPT-6 Astra / GPT-5.5 Pro / Gemini 3.8 Flash / Grok 4.20 Multi-Agent / Grok 4.5; plain rows that DO exist — Gemini 3.6 Flash, Gemini 3.1 Pro Preview, Gemini 3.5 Flash-Lite, Claude 4.5 Haiku — are all on the lowered scale of gotcha 34, so none were applied.)

30. **OpenAI pricing page also lists a full legacy/previous-generation catalog under "Flagship models" (discovered 2026-08-28)**: Three independent targeted fetches confirmed literal table rows for `gpt-5` ($1.25/$0.125/$10.00, batch $0.625/$5.00), `gpt-5.1` (identical to gpt-5), and `gpt-5.2` ($1.75/$0.175/$14.00, batch $0.875/$7.00) — plus, in the surrounding table context, `gpt-5.2-pro` ($21/$168), `gpt-4o` ($2.50/$10), `gpt-4o-2024-05-13` ($5/$15), and `gpt-4.1-nano` ($0.10/$0.40). None are labeled deprecated on the page. These predate our tracked GPT-5.4/5.5/5.6 family (and 5.3 Codex) in the same generational line the way `gpt-4o` predates `gpt-5` — i.e. previous-generation flagships still billable but superseded, analogous to Claude 3/Gemini 1.5/Grok 2/o-series, none of which we track either. Deliberately NOT added to pricing.json: this project tracks current-generation flagships across the four labs, not full historical catalogs. `gpt-5` is separately referenced in the web-search supported-models list above (line ~42) — that reference predates this discovery and doesn't imply it should be a pricing.json row. Re-raise as a scope question if the user wants full-catalog coverage; don't silently add these on a future run without an explicit decision. (2026-09-07: the catalog extraction has grown again — now also gpt-4.1, gpt-4.1-mini, gpt-4o-mini, o1, o1-pro, o3, o3-pro, o3-mini, o4-mini, gpt-4-turbo-2024-04-09, gpt-4-0613, gpt-3.5-turbo variants, davinci-002, babbage-002, plus a "Specialized models" chat-latest at $5/$0.50/$30 and gpt-5-search-api at $1.25/$0.125/$10. Whether these rows were always on the page or the extraction just reached further is unknown; either way, same scope decision applies — not added.)

31. **Gemini 3 Flash Preview extraction confused free-tier and paid-tier columns (2026-09-01)**: A targeted re-fetch asking specifically about "Gemini 3 Flash Preview" returned standard input/output/cache prices as "Free of charge" while reporting a normal-looking batch price ($0.25/$0.50 in). The batch figure matches our stored `batch_input_price` (0.25) exactly, so this reads as the extraction latching onto Google's free-tier quota column instead of the paid standard-rate column (same family of Gemini free/paid tier confusion as gotcha 5/6, not gotcha 9's context-tiering). Treated as an extraction artifact, not a real move to $0 — stored standard/cache values ($0.50/$3.00, cache $0.05) carried forward unchanged. Re-verify with a cleaner targeted fetch next audit rather than trusting a "Free of charge" reading for a model with no stated free-tier-only restriction. (Resolved 2026-09-04: a fetch that explicitly asked for the paid-tier column returned $0.50/$3.00, cache $0.05, batch $0.25/$1.50 — exact match to stored. Keep asking for "paid tier only" in the Google fetch prompt.)

32. **Claude Fable 5.1 / Mythos 5.1 cache reads are 0.025x, not 0.1x (added 2026-09-04)**: Anthropic's pricing page footnote: "Cache hits and refreshes on Claude Fable 5.1 and Claude Mythos 5.1 are priced at 0.025x the base input price. All other models use the standard 0.1x multiplier." So `cache_read_price` is $0.25 on a $10 input price, not $1.00. Read the "Cache hits and refreshes" column directly rather than deriving 10%. Both models otherwise match Fable 5 / Mythos 5 pricing ($10/$50, batch $5/$25). Mythos 5.1 is "limited availability" (Glasswing) like Mythos 5. Anthropic's models overview now files Fable 5, Opus 4.8/4.7/4.6/4.5, Sonnet 4.6/4.5 under "Legacy models (still available)" — that is NOT a deprecation label; none got `deprecated: true`. The overview's compare table also published Opus 5 max output (128K) for the first time — filled in from null. Fable 5.1's AA rows are all variant-suffixed (gotcha 29), so it carries a provisional 66 (Fable 5's 65 + 1). Mythos 5.1's predecessor (Mythos 5) has a null score, so 5.1 is null too, no provisional flag.

33. **GPT-6 Astra added 2026-09-04 — predecessor choice was a judgment call**: `gpt-6-astra` appears at the top of OpenAI's flagship table at $10/$1/$50 (batch $5/$25, LC in gotcha 18), described as "rolling out today for enterprises in our Trusted Access Program, with access through API ... coming in the coming days". "Astra" is a new tier name (vs Sol/Terra/Luna), but it sits in the flagship slot the way GPT-5.6 Sol did, so the provisional score uses Sol as predecessor (59 + 1 = 60; AA's variant rows read (high) 60 / (max) 61, consistent). Context window and max output are not on the pricing page — both stored null (didn't assume the GPT-5.x 128K max-output convention carries to a new generation). Web search: the page now says "Web search (all models): $10.00 / 1k calls", so `search_price` 0.010. Also noticed: the legacy catalog from gotcha 30 has grown (gpt-5-mini $0.25/$2, gpt-5-nano $0.05/$0.40, gpt-5-pro $15/$120 now also listed) — still deliberately not added.

34. **Artificial Analysis leaderboard shifted down ~9–12 points across ALL providers (first observed 2026-09-07)**: Two reads of `artificialanalysis.ai/leaderboards/models` this run showed every score uniformly lower than the 2026-09-04 reading — e.g. Claude Fable 5.1 (max with fallback) 66→57, Claude Opus 5 (high) 61→52, Claude Fable 5 (with fallback) 62→53, GPT-6 Astra (max) 61→55, GPT-5.6 Sol (max) →51, Gemini 3.8 Flash (high) 59→47, plain Gemini 3.6 Flash 52→40, Grok 4.6 (high) 61→51, Grok 4.5 (high) 56→45, Grok 4.3 (medium) →29. Relative ordering is preserved (Fable 5.1 still top, GPT-6 Astra second). The page shows no version/methodology notice, but a uniform cross-provider drop of this size is the signature of an Intelligence Index rebase (new evals added, scale re-normalised), not 40 models simultaneously getting dumber. Per the two-consecutive-observation rule, NO scores were rewritten this run. If the next audit shows the same lowered scale, the right move is a **wholesale** rebase of every stored `intelligence_score` from one consistent read (so relative sort order stays coherent), not a per-model drip — and provisional scores should be re-derived as predecessor+1 on the new scale. Do NOT mix old-scale and new-scale scores in `pricing.json`; a half-rebased table would sort wrongly. Raise to the user before doing the wholesale rewrite, since it changes every displayed score on the live site. **Second observation 2026-09-10: the scale is lower again, but not the SAME lowered scale** — Fable 5.1 (max with fallback) 66→57→53, GPT-6 Astra (max) 61→55→53, Opus 5 (high) 61→52→48, Fable 5 (with fallback) 62→53→50, GPT-5.6 Sol (max) →51→47, Grok 4.6 (high) 61→51→44, Grok 4.5 (high) 56→45→39, Gemini 3.8 Flash (high) 59→47→41, plain Gemini 3.6 Flash 52→40→34, Gemini 3.5 Flash (medium) →34, Grok 4.3 (medium) →29→25; plain rows we'd never seen lowered before also surfaced: Gemini 3.1 Pro Preview 30 (stored 57), Gemini 3.5 Flash-Lite 23 (stored 36), Claude 4.5 Haiku 18 (stored 37), o3 20. Relative ordering still preserved (Fable 5.1 = GPT-6 Astra (max) at the top). Because the numbers moved between the two lowered reads (57 vs 53 for the top model), the new scale hasn't stabilised yet — a wholesale rebase now would need redoing. Still no scores rewritten. Decision needed from the user (logged in to_do.md): (a) wait for two consecutive IDENTICAL lowered reads, then rebase wholesale from one read; or (b) rebase from the next read regardless. Either way, never mix scales.

35. **OpenAI "Cyber models" section (priced as of 2026-09-10; gpt-5.5-cyber first priced 2026-07-31, gpt-5.4-cyber first seen unpriced 2026-06-07)**: OpenAI's pricing page has a separate "Cyber models" table headed "Our latest Daybreak models." with rows `gpt-5.6-cyber` ($12.50 in / $1.25 cached / $15.625 cache writes / $75 out), `gpt-5.5-cyber` ($12.50 / $1.25 / — / $75), `gpt-5.4-cyber` (no prices), plus a `gpt-5.6-sol` row repeated as the "blue" counterpart. Note: "gpt-daybreak-blue-latest and gpt-daybreak-red-latest are aliases that currently point to gpt-5.6-sol and gpt-5.6-cyber, respectively." No access-restriction text on the pricing page itself, but these are security-specialised variants (red-team/blue-team pairing), not general flagship models — same scope reasoning as gotcha 30, so NOT added to pricing.json. Also note the "cache writes" column: OpenAI now publishes an explicit cache-write price for gpt-5.6-cyber and for the LC tiers of the GPT-6/5.6 family — a billing dimension our schema doesn't track (Anthropic has one too; we only store cache reads). Re-raise as a scope question if the user wants cyber variants or cache-write pricing.

36. **Gemini Omni Flash is a video-generation model, not a text model (confirmed 2026-09-10)**: Google's page section reads "Our next-generation video generation and editing model, now generally available to developers on the paid tier of the Gemini API." — model id `gemini-omni-1.1-flash`, $1.50 input (any modality), $9.00 text output / $17.50 video output. Previously carried on manual review as "Gemini Omni Flash Preview"; it has since GA'd. Out of scope for a text-token pricing table (same category as Gemini 3.5 Live Translate / Transcribe / 3.1 Flash Live Preview / Robotics ER). Don't add it just because it now has a text-output price.
